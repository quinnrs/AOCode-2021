...
"""
""" 
I am giving up of the frozenset sttempt for now
...
"""
 - 9/12 at 1500 results for digits is CORRECT but 
    I am stuck because frozenset objects are not subscripable and
    tuple objects are not callable

    I can visully map digit tuple '. . .' to s2d{'dab': 3} but I cannot
    wrire code to do it
...
def decode_digits(digits):
    digit_total = 0
    # for lin in fin:
    #     digits = list(map(lambda p: (frozenset(p)), digits))
    #     for frozenset in digits:
    #         print(f"frozenset in digits= ", frozenset)
        
    print(f"\nstarting a new solution loop")
    # s2d = decode_patterns(signals)
    for digit in digits: # these digitd are tuples
        print(f"digit in digits= ", digit)
        print(f"digits in d2s = ", digits)
        # digit_value = d2s[digit]
        # print(f"digit value =", digit_value)

        # print("TypeError: tuple indices must be integers or slices, not frozenset")
        
        # digit_value = d2s.get('digit') # cannot use get with a tuple object
